...
#else
#define TARGET_MEMORY   0x7FC8000
...
static uint32_t last_status = ERR_OK;


#if defined(DEBUG_TEST)
static const uart_cfg_t test_uart_cfg = {
    // Set Baud Rate
...
#define UART2_RX_PIN            GPIO_PIN_8

void test_uart_periph_init(uart_t *uart)
{
    // Initialize UART
    uart_initialize(uart, &test_uart_cfg);
...
#else
    *(volatile uint8_t *)(BASE_MEMORY + RESULT_OFFSET) = ACTION_CONTENTS;
    *(volatile uint32_t *)(BASE_MEMORY + RESULT_OFFSET + 4) = last_status;
#endif
}
...
void response_action(uint8_t *buffer, uint32_t result, uint8_t port_sel)
{
    ...
    send_packet(buffer, p - buffer);
#else
    *(volatile uint32_t *)(BASE_MEMORY + RESULT_OFFSET) = last_status;
#endif
}
...
void response_cfg_action_result(uint8_t *buffer, uint32_t result, uint8_t port_sel)
{
    ...
    *(volatile uint32_t *)(BASE_MEMORY + RESULT_OFFSET) = last_status;
    if (result != ERR_OK)
        *(volatile uint32_t *)(BASE_MEMORY + RESULT_OFFSET + 4) = result;
#endif
}
...
#else
    *(volatile uint32_t *)(BASE_MEMORY + RESULT_OFFSET) = last_status;
    *(volatile uint32_t *)(BASE_MEMORY + RESULT_OFFSET + 4) = (uint32_t)jedec_id;
#endif
}
...
static int8_t spi_flash_peripheral_init()
{
    ...
    // Check if there is an external SPI flash memory configuration
    if (spi_flash_cfg_p)
    {
        // Enable the SPI flash memory
        status = spi_flash_enable(&spi_cfg_gl, spi_flash_cfg_p);
    ...
    else
    {
        ...
        if (status != SPI_FLASH_ERR_OK)
        {
            // The device was not identified. The default parameters will be used.
...
uint8_t get_action(uint8_t *buffer)
{
    ...
    return buffer[0];
#else
    return *(volatile uint8_t *)(BASE_MEMORY + ACTION_OFFSET);
#endif
}
...
#else
    return *(volatile uint32_t *)(BASE_MEMORY + ADDRESS_OFFSET);
#endif
}
...
#else
    return *(volatile uint16_t *)(BASE_MEMORY + SIZE_OFFSET);
#endif
}

/*
 * The following variable must be placed just after the code.
 * The 1-byte variable can be initialized by an external tool.
 */
volatile uint8_t port_sel   __SECTION_ZERO("non_init");
...
#ifndef USE_UART
    *(volatile uint8_t *)(BASE_MEMORY + ACTION_OFFSET) = 0;
#endif
...
#else
    memset((void *) TARGET_MEMORY, 0, ALLOWED_DATA_SWD);
    *(volatile uint8_t *)(BASE_MEMORY + RESULT_OFFSET) = ACTION_READY;
#endif
    ...
    while (1) {
        ...
        switch (action)
        {
            ...
            case ACTION_SPI_GPIOS:
            {
                ...
                #else
                // jtag
                uint8_t * pin_buffer = (uint8_t *) TARGET_MEMORY;
                #endif
            ...
            case ACTION_GPIOS:
            {
                ...
                #else
                // jtag
                gpio_pin_cfg_t *pin_cfg = (gpio_pin_cfg_t *) TARGET_MEMORY;
                size /= 4;
                #endif
            ...
            case ACTION_SPI_INIT:
            {
                ...
                // Handle SPI flash memory protection based on the external tool configuration.
